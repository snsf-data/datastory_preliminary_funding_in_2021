...
__Erste vorläufige Zahlen zur SNF-Förderung im Jahr 2021: Wir haben 882 Millionen Franken in neue Projekte, Stipendien und Publikationen investiert.__

Die Hälfte der Mittel fliesst in die Projektförderung. Hier realisieren erfahrene Forschende ihre Ideen zu Themen, die sie selber gewählt haben. Mit einem Viertel des Budgets unterstützen wir Karrieren von jungen Forschenden, mit rund 20 Prozent die Arbeit von Forschungsgruppen in Programmen. Die übrigen Mittel verteilen sich auf Infrastrukturen und Wissenschaftskommunikation. Ende 2021 waren rund 5700 vom SNF finanzierte Projekte im Gang, mit 20'000 Forschenden an Hochschulen und weiteren Institutionen. Die meisten Projekte dauern mehrere Jahre.
...
<div class="plot-box">
<div class="plot-title">Bewilligte SNF-Förderbeiträge im Jahr 2021 (vorläufige Zahlen)</div>
...
draw_funding_portfolio_5 <- function(lang = "en") {
  ...
  tooltip_text <- funding_in_2021 %>% 
    group_by(FundingInstrumentGaLevel1, FundingInstrumentGaReporting_De) %>%
    summarise(scheme_granted = sum(AmountGranted, na.rm = TRUE),
              n = n()) %>% 
    ungroup() %>% 
    group_by(FundingInstrumentGaLevel1) %>% 
    arrange(desc(scheme_granted)) %>% 
    summarise(tooltip = paste0("<strong>", FundingInstrumentGaReporting_De, 
                               "</strong> (", 
                               print_num(round(scheme_granted / 1000000, 
                                               digits = 1)), 
                               " Mio. CHF zugesprochen in ", print_num(n), 
                               " Beiträgen)",
                               collapse = "<br>"))
...
girafe(ggobj = draw_funding_portfolio_5("de"), 
       height_svg = 2.5, 
       options = list(
...
```
<div class="caption">
Daten: <a href="https://github.com/snsf-data/datastory_preliminary_funding_in_2021/tree/main/data/prelim_funding_in_2021_20220131.csv" target="_blank">Bewilligte Beiträge im Jahr 2021 (vorläufige Daten, Stand 31. Januar 2022)</a>.
</div>
</div>
</div>
...
# Create ggiraph object
girafe(ggobj = draw_funding_portfolio_5_mob("de"), 
       width_svg = 3,
       height_svg = 6, 
       options = list(
         opts_toolbar(saveaspng = FALSE),
...
Daten und Code dieser Datengeschichte sind <a href="https://github.com/snsf-data/datastory_preliminary_funding_in_2021" target="_blank">auf Github verfügbar</a> und <a href="https://doi.org/10.5281/zenodo.6022479" target="_blank">auf Zenodo archiviert</a>.