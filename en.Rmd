...
<div class="plot-box">
<div class="plot-title">Approved SNSF grants in 2021 (preliminary figures)</div>
...
draw_funding_portfolio_5 <- function(lang = "en") {
  ...
  tooltip_text <- funding_in_2021 %>% 
    group_by(FundingInstrumentGaLevel1, FundingInstrumentGaReporting) %>%
    summarise(scheme_granted = sum(AmountGranted, na.rm = TRUE),
              n = n()) %>% 
    ungroup() %>% 
    group_by(FundingInstrumentGaLevel1) %>% 
    arrange(desc(scheme_granted)) %>% 
    summarise(tooltip = paste0("<strong>", FundingInstrumentGaReporting, 
                               "</strong> (", 
                               print_num(round(scheme_granted / 1000000, 
                                               digits = 1)), 
                               " million CHF approved in ", print_num(n), 
                               " grants)",
                               collapse = "<br>"))
...
girafe(ggobj = draw_funding_portfolio_5("en"), 
       height_svg = 2.5, 
       options = list(
...
```
<div class="caption">
Data: <a href="https://github.com/snsf-data/datastory_preliminary_funding_in_2021/tree/main/data/prelim_funding_in_2021_20220131.csv" target="_blank">Approved SNSF grants in 2021 (preliminary figures, 31 January 2022)</a>.
</div>
</div>
</div>
...
# Create ggiraph object
girafe(ggobj = draw_funding_portfolio_5_mob("en"), 
       width_svg = 3,
       height_svg = 6, 
       options = list(
         opts_toolbar(saveaspng = FALSE),
...
Data and code of this data story are <a href="https://github.com/snsf-data/datastory_preliminary_funding_in_2021" target="_blank">available on Github</a> and <a href="https://doi.org/10.5281/zenodo.6022479" target="_blank">archived on Zenodo</a>.